...
# 如果根目录不在sys.path中，则添加
if project_root not in sys.path:
    sys.path.insert(0, project_root)

# --- 应用程序的其余部分 ---
from validation.db_connector import get_db_engine
from test.core.data_fetcher.policy_data import get_policy_data
from test.core.data_fetcher.contract_data import get_latest_contract_data
from test.core.data_fetcher.financial_data import get_premium_collection_history
from test.core.calculations.iacf_calculator import build_iacf_timeline
from test.core.calculations.measure_unexpired_calculator import calculate_unexpired_measure
from test.core.data_fetcher.comparison_data import get_db_measure_result
...
env = st.sidebar.radio("请选择环境:", ('test', 'uat'), index=0)
                                    ...
                                    with st.expander(f"月份: {month_log['month']} 的计算详情"):
                                        ...
                                        if "亏损测试" in month_log['month']:
                                            ...
                                            if loss_pv_df is not None and not loss_pv_df.empty:
                                                ...
                                                # Check if DataFrame has 'month' column (new format) or is already transposed (old format)
                                                if 'month' in loss_pv_df.columns:
                                                    # New format: transpose it
                                                    ...
                                                    # Old format: already transposed
                                                    st.dataframe(loss_pv_df.style.format('{:.4f}'))
                                                    st.markdown(f"**折现值合计 (PV):** `{loss_pv_df.loc['折现值'].sum():.4f}`")
                                            ...
                                            if maintenance_pv_df is not None and not maintenance_pv_df.empty:
                                                ...
                                                # Check if DataFrame has 'month' column (new format) or is already transposed (old format)
                                                if 'month' in maintenance_pv_df.columns:
                                                    # New format: transpose it
                                                    ...
                                                    # Old format: already transposed
                                                    st.dataframe(maintenance_pv_df.style.format('{:.4f}'))
                                                    st.markdown(f"**折现值合计 (PV):** `{maintenance_pv_df.loc['折现值'].sum():.4f}`")
                            except Exception as e:
                                st.error(f"计量计算失败: {e}")
                                import traceback
